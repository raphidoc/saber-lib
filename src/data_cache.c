...
static size_t r_rs_b_n_class = 0;
...
static void free_class_names(void)
{
    if (r_rs_b_class_names) {
        for (size_t j = 0; j < r_rs_b_n_class; ++j)
                ...
                size_t         n_class)
{
    ...
    double *tmp_wl     = realloc(r_rs_b_wl,     sizeof(double) * n_wl);
    double *tmp_matrix = realloc(r_rs_b_matrix, sizeof(double) * n_wl * n_class);
    if (!tmp_wl || !tmp_matrix) return 1;

    char **tmp_names = malloc(sizeof(char*) * n_class);
    ...
    r_rs_b_n_class    = n_class;

    memcpy(r_rs_b_wl,     wl,     sizeof(double) * n_wl);
    memcpy(r_rs_b_matrix, matrix, sizeof(double) * n_wl * n_class);
...
int build_cache(const double* wl, size_t n) {
    ...
    cached_r_rs_b = realloc(cached_r_rs_b, sizeof(double) * n * r_rs_b_n_class);

    interpolate_vector(a0a1_wl, a0_val, wl, cached_n_wl, cached_n_wl, cached_a0);
    interpolate_vector(a0a1_wl, a1_val, wl, cached_n_wl, cached_n_wl, cached_a1);
    interpolate_vector(a_w_wl, a_w_val, wl, cached_n_wl, cached_n_wl, cached_a_w);
    interpolate_matrix(r_rs_b_wl, r_rs_b_matrix, wl, cached_n_wl, cached_n_wl, r_rs_b_n_class, cached_r_rs_b);
...
size_t get_n_class()      { return r_rs_b_n_class; }
